...
You can exclude specific agents from discovery using glob patterns in the `agents.ignore` option. This is useful for filtering out draft agents, test agents, or archived agents that you don't want to appear in the agent list.

```jsonc title="opencode.jsonc"
{
  "$schema": "https://opencode.ai/config.json",
  "agents": {
    "ignore": ["drafts/**", "test-*", "archive/**"],
  },
}
```

The patterns are matched against agent names, which are derived from their file paths. For example, an agent defined in `.opencode/agents/drafts/my-agent.md` has the name `drafts/my-agent`. Patterns follow glob syntax:
